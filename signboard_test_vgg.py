# -*- coding: utf-8 -*-
"""signboard_test_vgg-2.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1P__hUStbK0TeqCZrkQgKzhzymb5Rj2S1
"""
...
test_path = 'drive/My Drive/BTPS^3/Signboard Annotation/BSVSO Datasets/test_annotation.txt'  # Test data (annotation file)

test_base_path = 'drive/My Drive/BTPS^3/Signboard Annotation/BSVSO Datasets/Test Data/' # Directory to save the test images
...
bbox_threshold = 0.7 # 0.5

with open(config_output_filename, 'rb') as f_in:
	C = pickle.load(f_in)
'''
C.model_path = base_path + 'Signboard CNN/'+ network +'/'+str(Train_V)+'/model_frcnn_'+ network +'.hdf5'
C.rcnn_model = base_path + 'Signboard CNN/'+ network +'/BN_Exp_A/127ep_model_frcnn_vgg16.hdf5'
C.num_rois = 4
C.record_path = base_path +'Signboard CNN/'+ network +'/'+str(Train_V)+'/record.csv'
C.anchor_box_scales = [71,104,129]
C.anchor_box_ratios = [[2.37,1],[2.46,1],[3.29,1]]
C.im_size = 600
C.img_channel_mean = [99.323 , 97.898, 94.887]
C.img_scaling_factor = 1.0
C.rpn_stride = 16
...
mAP_record_path = base_path +str(Train_V)+'/mAP_rec.csv'

if not os.path.isfile(mAP_record_path):
  print("Created")
  mAP_df = pd.DataFrame(columns=['Epoch', 'mAP', 'Max'])
  MaX = 0
else:
  print("Loaded")
  mAP_df = pd.read_csv(mAP_record_path)
  mAP_epochs = len(mAP_df)
  MaX = mAP_df.at[mAP_epochs-1, "Max"]
  print('Max MeanAP: ',MaX )
...
while True:
  record_df = pd.read_csv(C.record_path)
  Cur_Epoch = len(record_df)
  #print('Current Epoch:',Cur_Epoch)
  sys.stdout.write('\r'+'Current Epoch:' + str(Cur_Epoch))
  if earlystop==6:
    print("\nearlystop triggered...")
    if os.path.isfile(C.model_path):    
      print("\nModel Renamed")
      dest = base_path +str(Train_V) +'/Earlystop_' + str(Cur_Epoch)+ '_frcnn_'+ network +'.hdf5'
      os.rename(C.model_path, dest)
    earlystop=0
  if ((Cur_Epoch) % 3)==False and os.path.isfile(C.model_path)  and os.path.isfile(C.record_path) and done_ep != Cur_Epoch :
    st = time.time()
    print("\nValidation triggered...at Epoch {}".format(Cur_Epoch, num_epochs))
    #reset_weights(model_rpn_)
    #reset_weights(model_classifier_)
    load_val_model()
    map_val = calc_mAP(test_imgs,model_rpn,model_classifier,C,class_mapping,Max_boxes,rpn_ov_thresh,nms_ov_thresh)
    meanAP = round(np.mean(np.array(map_val)), 3)
    del map_val
    if meanAP>MaX: 
      MaX=meanAP 
      earlystop = 0
    if meanAP<MaX: 
      earlystop+=1
    new_row = {'Epoch':(Cur_Epoch), 'mAP':(meanAP), 'Max':(MaX)}
    mAP_df = mAP_df.append(new_row, ignore_index=True)
    mAP_df.to_csv(mAP_record_path, index=0)
    print('\nValidation mAP at Epoch {} = {}'.format(Cur_Epoch, meanAP))
    done_ep = Cur_Epoch
    print('\nSpend %0.2f mins to validated the model' % ((time.time()-st)/60) )
  if (Cur_Epoch==num_epochs): 
    print('\nThis is last training Epoch {}/{} \nValidation Complete'.format(Cur_Epoch, num_epochs))
    break
...
for idx, img_name in enumerate(imgs_path):
    ...
    # R.shape = (300, 4)
    R = rpn_to_roi(Y1, Y2, C, K.image_dim_ordering(),overlap_thresh=0.9)
