...
def calc_mAP(test_imgs,model_rpn,model_classifier,C,class_mapping,Max_boxes,rpn_ov_thresh,nms_ov_thresh):
  T = {}
  P = {}
  mAPs = []
  for idx, img_data in enumerate(test_imgs):
      ...
      R = rpn_to_roi(Y1, Y2, C, K.image_dim_ordering(), max_boxes=Max_boxes, overlap_thresh= rpn_ov_thresh)
      ...
      for key in bboxes:
          ...
          new_boxes, new_probs = non_max_suppression_fast(bbox, np.array(probs[key]), overlap_thresh=nms_ov_thresh, max_boxes=Max_boxes)
      ...
      for key in T.keys():
          ...
          all_aps.append(ap)
      sys.stdout.write('\r'+'mAP =' + str(np.mean(np.array(all_aps))))
