...
def main():
    ...
    if not os.path.isfile('signboard_detector.py'):
      gdd.download_file_from_google_drive(file_id='1V479J_jiHbZ7ZNAPeJCNACrSCCiRQsUS', dest_path='./Signboard_Detector.zip', unzip=True)
    ...
    import signboard_detector as fn
...
import time	
from google.colab import files
from Signboard_Detector import functions as fn
class Config:

  def __init__(self):
    self.verbose = True

def detect(test_file_path, model_path, config_file_path):
    st = time.time()
    #with open(config_file_path, 'rb') as f_in:
    #  C = pickle.load(f_in)
    anchor_box_scales = [60,100,140,250]  #[71,104,129,418]
    anchor_box_ratios =  [[3,1],[5,1],[8,1],[7,2]]  #[[2.37,1],[2.46,1],[3.26,1],[4.7,1]]
    #im_size = 600
    #img_channel_mean =  [99.323 , 97.898, 94.887]
    ...
    rpn_layers = fn.rpn_layer(shared_layers, num_anchors)
    classifier = fn.classifier_layer(feature_map_input, roi_input, num_rois, nb_classes=len(class_mapping))
    model_rpn = Model(img_input, rpn_layers)
    model_classifier_only = Model([feature_map_input, roi_input], classifier)
    model_classifier = Model([feature_map_input, roi_input], classifier)
    # Switch key value for class mapping
    ...
    record_path = "output.csv"
    if not os.path.isdir(test_file_path):
      print('Warning ! "Input" folder is not found..\nPlease upload your input zip file with mentioned format and run the tool again.')
    if not os.path.isfile(model_path) or not os.path.isfile(config_file_path) :
      print('Warning ! Detection model file is not found..\nPlease download Signbord_Classifier.zip and run the tool agian.')
...
test_path = "drive/My Drive/BTPS^3/Signboard Annotation/SVSO Datasets/Test Data"
model_path = "drive/My Drive/BTPS^3/Signboard Annotation/tfSignB-1/model_frcnn_vgg16-ft.hdf5"
detect(test_path, model_path)
'''