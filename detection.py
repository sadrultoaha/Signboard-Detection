...
def main()
    ...
    if not os.path.isdir(test_file_path)
      print('Warning ! Input folder is not found..nPlease upload your input zip file with mentioned format and run the tool again.')
    if not os.path.isfile(model_file_path) or not os.path.isfile(config_file_path) 
      print('Warning ! Detection model file is not found..nPlease download Signbord_Classifier.zip and run the tool agian.')
    ...
    for idx, img_name in enumerate(imgs_path)
        ...
        for jk in range(R.shape[0]C.num_rois + 1)
            ROIs = np.expand_dims(R[C.num_roisjkC.num_rois(jk+1), ], axis=0)
            if ROIs.shape[1] == 0
                break
            ...
            for ii in range(P_cls.shape[1])
                ...
                try
                    (tx, ty, tw, th) = P_regr[0, ii, 4cls_num4(cls_num+1)]
                    ...
                    pass
                bboxes[cls_name].append([C.rpn_stridex, C.rpn_stridey, C.rpn_stride(x+w), C.rpn_stride(y+h)])
        ...
        for key in bboxes
            ...
            for jk in range(new_boxes.shape[0])
                ...
                textLabel = '{} {}'.format(key,int(100new_probs[jk]))
                all_dets.append((key,100new_probs[jk]))
                ...
                new_row = {'File_Name'img_name, 
                      ...
                      'Label_Name'key,
                      'Confidence_Score'round(100new_probs[jk],2)
                      }
    ...
    zipObj.close()
    print('Detection complete ! output results have been created and archived into Result.zip filenPlease approve the download.')
...
if __name__ == __main__
    '''
    Usage python detect.py -test_file_path Signboard-DetectionTest Data -output_csv output.csv -output_zip result.zip
    '''
    main()
